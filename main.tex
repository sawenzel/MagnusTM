...
\vspace{0.2cm}
Puisque $a = \frac{dv}{dt}$, l'équation~\ref{skier_acc_airdrag} est en réalité une \textbf{équation différentielle} du premier ordre. En posant la vitesse terminale $v_\infty = \sqrt{\dfrac{2mg\sin\alpha}{C\rho A}}$ et une constante de temps $\tau$, sa solution analytique (avec $v(0)=0$) est:
$$v(t) = v_\infty\!\left(1 - e^{-t/\tau}\right)$$
La vitesse croît depuis zéro et s'approche asymptotiquement de~$v_\infty$ — la vitesse à laquelle la résistance de l'air compense exactement la composante gravitationnelle. En pratique, le skieur ne l'atteint jamais sur un tracé de longueur finie.
